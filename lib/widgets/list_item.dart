...
import '../widgets/list_item_attribute.dart';
  ...
  Widget build(BuildContext context) {
    ...
    if(attributes.length <= 3 && attributes.isNotEmpty) {
      for (var element in attributes) {
        ...
        rows.add(const TableRow(children: [SizedBox(height: MarginValue.small,)]));
      }
    } else if(attributes.length > 3) {
      rows.add(TableRow(children: [attributes[0],const SizedBox()]));
      rows.add(const TableRow(children: [SizedBox(height: MarginValue.small,), SizedBox(height: MarginValue.small,)]));

      int count = 0;
      List<Attribute> attributePerRow = [];
